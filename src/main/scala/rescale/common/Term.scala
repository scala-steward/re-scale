 ...
 *
 * Terminal output helpers with ANSI colors. Plain-text fallback when
 * NO_COLOR is set or TERM is empty. Emits diagnostics to stderr so
 * that redirected stdout (e.g. piping to jq or wc -l) stays clean.
 *
 * Kept as pure String functions for coloring + side-effecting Unit
 * methods for logging. Not an Effect type — logging is universal
 * enough that wrapping it in IO adds noise without safety gain.
 */
...
object Term {
  ...
  /** Format a simple table with aligned columns. Widths are set by the
    * longest cell in each column. Safe for any width terminal — no
    * wrapping, just truncation at the longest cell.
    */
  def table(headers: List[String], rows: List[List[String]]): String = {
    ...
    allRows
      .map { row =>
        row
          .zip(colWidths)
          .map { case (cell, width) => cell.padTo(width, ' ') }
          .mkString("  ")
      }
      .mkString("\n")
  }
